{**
 * plugins/themes/lrjstm/templates/frontend/objects/article_summary.tpl
 *
 * Article list item. Based on templates/frontend/objects/article_summary.tpl
 * with the same author, access and galley logic and the
 * Templates::Issue::Issue::Article hook. Adds a short abstract excerpt
 * (hide it with hideAbstract=true) and a two-column layout when the article
 * has a cover image.
 *
 ...
 * @uses $hideAbstract bool Hide the abstract excerpt?
...
<div class="obj_article_summary lr-summary{if $publication->getLocalizedData('coverImage')} has_cover{/if}">
	...
	<div class="lr-summary__body">
		...
		{if !$hideAbstract}
			{assign var=summaryAbstract value=$publication->getLocalizedData('abstract')|strip_unsafe_html|strip_tags|trim}
			{if $summaryAbstract}
				<p class="lr-summary__abstract">{$summaryAbstract|truncate:320:"…"}</p>
			{/if}
		{/if}
