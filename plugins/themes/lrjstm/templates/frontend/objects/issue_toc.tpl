{**
 * plugins/themes/lrjstm/templates/frontend/objects/issue_toc.tpl
 *
 * Based on templates/frontend/objects/issue_toc.tpl. Changes: when
 * $lrjstmIssueHeader is set (issue.tpl already shows the cover, description
 * and date) those parts are skipped here, and the article sections get a
 * "Table of Contents" heading with the #lrIssueToc anchor. Everything else is
 * unchanged.
 *
 ...
 * @uses $lrjstmIssueHeader bool Cover, description and date are shown by the page
 *}
{if !$heading}
...
<div class="obj_issue_toc">
	...
	<div class="heading">
		...
		{if $issueCover && !$lrjstmIssueHeader}
			<div class="cover">
		...
		{* Description *}
		{if $issue->hasDescription() && !$lrjstmIssueHeader}
		...
		{* Published date *}
		{if $includeIssuePublishDate && $issue->getDatePublished() && !$lrjstmIssueHeader}
			<div class="published">
				<span class="label">
	...
	{* Articles *}
	{if $lrjstmIssueHeader}
		<h2 class="lr-toc__title" id="lrIssueToc">{translate key="issue.toc"}</h2>
	{/if}
	<div class="sections">
